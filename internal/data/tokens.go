...
func generateToken(userID int64, ttl time.Duration, scope string) (*Token, error) {
	...
	// fill the byte slice with random bytes from OS's CSPRNG
	...
	// remove padded '=' character at the end using WithPadding(base32.NoPadding)
	token.Plaintext = base32.StdEncoding.WithPadding(base32.NoPadding).EncodeToString(randomBytes)
	// generate SHA-256 hash of the plaintext token string
	hash := sha256.Sum256([]byte(token.Plaintext))
	// convert array of length 32 returned from sha256.Sum256 to slice
	token.Hash = hash[:]
